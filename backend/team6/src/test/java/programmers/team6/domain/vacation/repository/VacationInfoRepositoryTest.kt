...
class VacationInfoRepositoryTest {
    ...
    @Autowired
    private lateinit var memberFactory: TestMemberFactory
    ...
    fun `연차대상자_검색`(now: LocalDate, count: Int) {
        ...
        val info = vacationInfoRepository.save(VacationInfo(15.0, 0.0, "testType", member.id))

        val result = vacationInfoRepository.findAnnualVacationFrom(now.minusYears(1), now)

        assertThat(result).hasSize(count)
        if (count == 1) {
            assertThat(info).isEqualTo(result.first())
        }
    }
